...
export const useFormComponentStyles = <TModel>(
  ...
  // Calculate dimensions with canvas width only when in designer mode
  const canvasWidth = shaForm.formMode === "designer" ? designerWidth : undefined;
  const dimensionsStyles = useMemo(() =>
    getDimensionsStyle(dimensions, canvasWidth),
    [dimensions, canvasWidth, shaForm.formMode]
  );
